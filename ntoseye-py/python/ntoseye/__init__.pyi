...
class Debugger:
    ...
    def step_over(self) -> dict[str, Any]:
        ...
        single-step). Blocks until done; returns the same shape as `run()`
        (`{stop: "step"|"breakpoint"|...}`). Requires the VM halted."""
        ...
    def step_out(self) -> dict[str, Any]:
        """Step out of the current function (run to the caller's return address).
        Blocks until done; returns the same shape as `run()`. Requires the VM
        halted."""
        ...
    ...
    def status(self) -> dict[str, Any]:
        ...
        stale addresses."""
        ...
    def wait_for_stop(self, timeout_ms: int | None = None) -> dict[str, Any]:
        """Wait for the next stop WITHOUT resuming, up to `timeout_ms` (None blocks,
        polling for KeyboardInterrupt). Returns the same shape as `run()`, plus
        `{stop: "halted"}` immediately if the VM is already parked with nothing
        pending. Does not resume; use `cont()` (or `run()`) to advance."""
        ...
    def run(self, timeout_ms: int | None = None) -> dict[str, Any]:
        """Resume and wait for the next meaningful stop, auto-resuming past
        wrong-process and false-conditional breakpoint hits. Returns
        `{stop: "breakpoint"|"exception"|"bugcheck"|"target_reloaded"|"running", ...}`.
        With `timeout_ms` set, returns `{stop: "running"}` if nothing stopped in
        that window (poll again); with `None`, blocks until a stop."""
        ...
    ...
    def set_breakpoint(self, addr: int, condition: str | None = None) -> int:
        ...
        """
        ...
    def clear_breakpoint(self, id: int) -> None:
        """Clear a breakpoint. Requires the VM halted."""
        ...
    def enable_breakpoint(self, id: int) -> None:
        """Re-arm a disabled breakpoint. Requires the VM halted."""
        ...
    def disable_breakpoint(self, id: int) -> None:
        """Disable a breakpoint without forgetting it (re-enable later).
        Requires the VM halted."""
        ...
    def breakpoints(self) -> list[dict[str, Any]]:
        """Breakpoints as dicts `{id, address, enabled, symbol, scope,
        condition, temporary}` (`scope` is `"global"` or `"name (pid)"`)."""
        ...
